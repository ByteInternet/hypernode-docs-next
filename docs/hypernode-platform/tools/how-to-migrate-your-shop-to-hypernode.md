...
\*Although the importer is a very easy way to import a shop, please keep in mind that it uses the command line. So if needed, find yourself an [experienced developer](https://www.hypernode.com/en/our-partners/) that can assist you with these tasks.

## Migrate Your Shop via Shell Using the `hypernode-importer`

Please use the Hypernode Importer tool via Shell.
