...
Beside above known plugins that will hit the blacklisted keyword in the `http.ratelimit` we know that Picqer will hit the rate limiter as well because of being blocked by "**zoneperip**". Please find [here](https://picqer.com/files/ip-addresses.txt) the IP adresses of Picqer. You're able to exclude those IP's from hitting the rate limiter if you follow the [instructions](#Known-Rate-Limited-Plugins-and-Service-Provider).
...
**Please note:** if you have [Hypernode Managed Vhosts](https://support.hypernode.com/en/hypernode/nginx/hypernode-managed-vhosts) enabled, only add the `http.conn_ratelimit` file in the Nginx root. Don't add it to the specific vhost as well, as these files will cancel each other out.
