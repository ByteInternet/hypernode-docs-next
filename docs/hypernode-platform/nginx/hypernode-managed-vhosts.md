...
Using Varnish works slightly different with HMV enabled. Of course Varnish needs to be enables with the **systemctl** tool. `hypernode-systemctl settings varnish_enabled True`
...
Once the command is processed you could list all the vhost to check if Varnish is enabled for that Vhost. The value in the Varnish column should be set to **True**.
